...
async function upsertWriter(input) {
...
async function upsertBook(authorId, slug, title) {
    ...
    const premiumChapter = await prisma.chapter.upsert({
        ...
        create: {
            ...
            content: "A deterministic premium chapter for controlled economy acceptance.",
            status: "DRAFT",
            accessType: "PREMIUM",
            price: 120,
            wordCount: 10,
...
async function upsertPublishedPremiumBook(authorId) {
    ...
    const book = await prisma.book.upsert({
        ...
        create: {
            ...
            slug,
            synopsis: "Controlled published premium book for real economy attribution.",
            status: "PUBLISHED",
    ...
    const chapter = await prisma.chapter.upsert({
        ...
        create: {
            ...
            content: "A published premium chapter for controlled earnings acceptance.",
            status: "PUBLISHED",
            publishedAt: new Date(),
            accessType: "PREMIUM",
            price: 120,
...
async function main() {
    ...
    const bookA = await upsertBook(writerA.id, fixture.writerA.slug, "Phase 7F Acceptance Book A");
    const bookB = await upsertBook(writerB.id, fixture.writerB.slug, "Phase 7F Acceptance Book B");
