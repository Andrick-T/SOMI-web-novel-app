...
async function getFixtureSnapshot(email) {
...
async function ensureFixturePassword(email) {
    ...
    if (JSON.stringify(preserved) !== JSON.stringify(current)) {
        throw new Error(`Economy state changed while setting fixture password for ${email}: before=${JSON.stringify(preserved)} after=${JSON.stringify(current)}`);
    }
    return { before, after };
}
async function main() {
    console.log("[phase7e-fixture] LOCAL PHASE 7E ACCEPTANCE ONLY");
    console.log("[phase7e-fixture] Reusing existing auth password hashing implementation from backend/src/modules/auth/auth.crypto.ts");
    const snapshots = {};
    ...
    console.log("[phase7e-fixture] Fixture password reset complete. Existing user IDs, wallets, transactions, entitlements and reading state remain unchanged.");
}
try {
    await main();
}
