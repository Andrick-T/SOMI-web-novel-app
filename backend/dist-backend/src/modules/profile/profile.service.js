...
const toUtcDay = (date) => date.toISOString().slice(0, 10);
const getPreviousUtcDay = (date) => {
...
const calculateDayStreak = (dates) => {
    ...
    const uniqueDays = Array.from(new Set(dates.map(toUtcDay))).sort((a, b) => b.localeCompare(a));
...
const calculateReadingGenres = (progress) => {
    const booksById = new Map();
    for (const entry of progress) {
        if (!booksById.has(entry.bookId)) {
            booksById.set(entry.bookId, entry.book.genres.map((item) => item.genre.name));
        }
    }
...
const getProfileAccount = async (userId) => {
    ...
    try {
        await prisma.$transaction(async (tx) => {
            ...
            const existingPreferences = existingProfile?.preferences &&
                ...
                : {};
            const mergedPreferences = input.preferences !== undefined
                ? {
                    ...existingPreferences,
                    ...input.preferences,
                }
        ...
        if (error instanceof Prisma.PrismaClientKnownRequestError &&
            error.code === "P2002") {
            throw new AppError(409, "USERNAME_ALREADY_EXISTS", "That username is already in use.");
        }
        throw error;
    }
