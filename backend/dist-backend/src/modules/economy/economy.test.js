...
import { creditWalletFromTrustedPayment, getCoinsForCustomPurchase, } from "./economy.service.js";
const app = createApp();
...
const createPremiumChapter = async (ownerId) => {
...
describe("Phase 7E economy integration", () => {
    ...
    it("uses exact package pricing, integer-safe custom pricing, and payment idempotency", async () => {
        ...
        await expect(creditWalletFromTrustedPayment({
            userId: reader.userId,
            amountCfa: 99,
            providerReference: `payment-${randomUUID()}`,
            verified: true,
    ...
    it("unlocks at the persisted chapter price, records the ledger, and is idempotent", async () => {
        ...
        expect(debit[0].metadata).toEqual({ balanceBefore: 3150, balanceAfter: 3030 });
    ...
    it("protects insufficient funds, IDOR, and concurrent duplicate unlocks", async () => {
        ...
        expect([one.status, two.status].every((status) => status === 200 || status === 409)).toBe(true);
