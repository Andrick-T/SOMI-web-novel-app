...
describe("Phase 7D library and reading progress API", () => {
    ...
    it("requires authentication and validates progress input", async () => {
        ...
        expect((await request(app)
            ...
            .set("Authorization", `Bearer ${readerA.token}`)).status).toBe(400);
        expect((await request(app)
            .put(`/api/v1/books/${bookA.id}/chapters/${chapterA.id}/reading-progress`)
            .set("Authorization", `Bearer ${readerA.token}`)
            .send({ progressPercent: 101, position: 0 })).status).toBe(400);
        expect((await request(app)
            .put(`/api/v1/books/${bookA.id}/chapters/${chapterA.id}/reading-progress`)
            .set("Authorization", `Bearer ${readerA.token}`)
            .send({ progressPercent: 10, position: -1 })).status).toBe(400);
    });
    it("persists idempotent library membership and isolates users", async () => {
        ...
        expect(await prisma.libraryItem.count({
            where: { userId: readerA.id, bookId: bookA.id },
        })).toBe(1);
        expect((await request(app)
            .get("/api/v1/library")
            .set("Authorization", `Bearer ${readerB.token}`)).body.items).toHaveLength(0);
        expect((await request(app)
            .delete(`/api/v1/library/${bookA.id}`)
            .set("Authorization", `Bearer ${readerA.token}`)).status).toBe(204);
        expect(await prisma.book.findUnique({ where: { id: bookA.id } })).not.toBeNull();
    });
    it("persists, retrieves, isolates, and rejects cross-book progress", async () => {
        ...
        expect((await request(app)
            ...
            .set({ Authorization: `Bearer ${readerB.token}` })).body.items).toHaveLength(0);
        expect((await request(app).get("/api/v1/reading-progress/continue").set(auth))
            .body.item.chapterId).toBe(chapterA.id);
    });
    it("keeps newer server state over an older client state", async () => {
        ...
        expect(await prisma.readingProgress.findUnique({
            where: {
