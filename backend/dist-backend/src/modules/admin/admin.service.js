...
const clampPage = (page) => Math.max(1, page);
const clampLimit = (limit) => Math.min(100, Math.max(1, limit));
 ...
 */
const mapAuditMetadata = (metadata) => {
  if (!metadata || typeof metadata !== "object" || Array.isArray(metadata)) {
    return null;
  }
  return metadata;
};
async function getAdminEconomyStats() {
  const [
    totalCoinPurchases,
    totalCoinsSpent,
    revenue,
