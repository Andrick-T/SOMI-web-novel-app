...
adminRouter.get("/users", validate(adminQuerySchema, "query"), asyncRoute(async (req, res) => {
...
adminRouter.get("/writers", validate(adminWriterQuerySchema, "query"), asyncRoute(async (req, res) => {
...
adminRouter.get("/economy", asyncRoute(async (_req, res) => {
    res.json(await getAdminEconomySummary());
}));
adminRouter.get("/transactions", validate(adminTransactionQuerySchema, "query"), asyncRoute(async (req, res) => {
    const { search, type, status, userId, page, limit } = req.query;
...
adminRouter.get("/books", validate(adminBookQuerySchema, "query"), asyncRoute(async (req, res) => {
    const { search, status, writerId, page, limit } = req.query;
...
adminRouter.patch("/books/:bookId", validate(updateBookSchema), asyncRoute(async (req, res) => {
    ...
    if (req.body?.status === "PUBLISHED" ||
        ...
        const action = req.body.status === "PUBLISHED" ? "BOOK_PUBLISHED" : "BOOK_UNPUBLISHED";
...
adminRouter.get("/books/:bookId", asyncRoute(async (req, res) => {
...
adminRouter.get("/audit", validate(adminAuditQuerySchema, "query"), asyncRoute(async (req, res) => {
    const { search, action, targetType, page, limit } = req.query;
