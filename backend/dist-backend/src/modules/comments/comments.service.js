...
const commentModel = prisma.comment ?? prisma.comments;
...
const getPublishedChapter = async (bookId, chapterId) => {
...
export async function createChapterComment(user, bookId, chapterId, content, parentId, gifId, gifUrl) {
    ...
    if ((normalizedGifId && !normalizedGifUrl) ||
        (!normalizedGifId && normalizedGifUrl)) {
        throw new AppError(400, "INVALID_GIF", "GIF ID and GIF URL must be provided together.");
    }
    if (normalizedGifUrl) {
        try {
            const url = new URL(normalizedGifUrl);
            const hostname = url.hostname.toLowerCase();
            const isAllowedGiphyHost = hostname === "media.giphy.com" ||
    ...
    if (normalizedParentId) {
        ...
        if (!parentComment) {
            throw new AppError(400, "INVALID_PARENT_COMMENT", "The parent comment does not belong to this chapter.");
        }
    }
...
export async function deleteChapterComment(user, bookId, chapterId, commentId) {
    const currentUser = requireUser(user);
