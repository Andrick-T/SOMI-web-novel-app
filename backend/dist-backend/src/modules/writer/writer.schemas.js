...
export const richContentSchema = z
    ...
    .refine((value) => !unsafeContentPattern.test(JSON.stringify(value)), "Content contains unsafe markup or URLs.");
/**
 * Editable book-localization fields only.
 *
 * IMPORTANT:
 * `status` is intentionally excluded.
 *
 * Writers must not be able to forge:
 * READY_FOR_SUBMISSION
 *
 * Readiness is handled through the dedicated
 * POST /writer/books/:bookId/localizations/:languageCode/ready
 * transition.
 */
...
/**
 * Metadata used by the binary upload endpoint.
 *
 * Kept separate from assetSchema because the MIME type,
 * size and binary payload come from the actual uploaded file,
 * not from trusted JSON supplied by the client.
 */
