...
const assertWriterCannotChangeLifecycleStatus = (req, status, resource) => {
    if (req.user?.role.toUpperCase() === "WRITER" && status !== undefined) {
        throw new AppError(409, "INVALID_STATE_TRANSITION", `Writers cannot directly change ${resource} lifecycle status. Submit the ${resource} through the Writer workflow.`);
    }
};
...
contentRouter.get("/writer/books", requireAuth, requireRole("WRITER", "ADMIN"), asyncRoute(async (req, res) => {
    const books = await getWriterBooks(req.user.id);
    res.json({ books });
}));
contentRouter.post("/books", requireAuth, requireRole("WRITER", "ADMIN"), validate(createBookSchema), asyncRoute(async (req, res) => {
    /*
...
contentRouter.get("/books/:bookId", optionalAuth, asyncRoute(async (req, res) => {
    const bookId = String(req.params.bookId);
    const book = await getBookDetail(bookId, req.user);
    res.json({ book });
}));
contentRouter.patch("/books/:bookId", requireAuth, validate(updateBookSchema), asyncRoute(async (req, res) => {
    assertWriterCannotChangeLifecycleStatus(req, req.body.status, "book");
    const bookId = String(req.params.bookId);
    const book = await updateBook(req.user, bookId, req.body);
    res.json({ book });
}));
contentRouter.delete("/books/:bookId", requireAuth, asyncRoute(async (req, res) => {
    const bookId = String(req.params.bookId);
    await deleteBook(req.user, bookId);
    res.status(204).send();
}));
contentRouter.get("/books/:bookId/chapters", asyncRoute(async (req, res) => {
    const bookId = String(req.params.bookId);
    const chapters = await getPublicChapters(bookId);
    res.json({ chapters });
}));
contentRouter.get("/books/:bookId/chapters/manage", requireAuth, requireRole("WRITER", "ADMIN"), asyncRoute(async (req, res) => {
    const bookId = String(req.params.bookId);
    const chapters = await getWriterChapters(req.user, bookId);
    res.json({ chapters });
}));
contentRouter.post("/books/:bookId/chapters", requireAuth, requireRole("WRITER", "ADMIN"), validate(createChapterSchema), asyncRoute(async (req, res) => {
    /*
...
contentRouter.get("/books/:bookId/chapters/:chapterId", optionalAuth, asyncRoute(async (req, res) => {
    const bookId = String(req.params.bookId);
    const chapterId = String(req.params.chapterId);
    const chapter = await getChapterDetail(bookId, chapterId, req.user);
    res.json({ chapter });
}));
contentRouter.patch("/books/:bookId/chapters/:chapterId", requireAuth, validate(updateChapterSchema), asyncRoute(async (req, res) => {
...
contentRouter.delete("/books/:bookId/chapters/:chapterId", requireAuth, asyncRoute(async (req, res) => {
    const bookId = String(req.params.bookId);
    const chapterId = String(req.params.chapterId);
