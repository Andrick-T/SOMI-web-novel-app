...
const useApiContent = appConfig.useApiContent;
...
class ApiChapterRepository {
  ...
  async getChaptersByBookId(bookId: string): Promise<Chapter[]> {
    try {
      ...
      this.error =
        error instanceof Error ? error : new Error("Failed to load chapters.");
  ...
  async getChapterById(
    ...
    try {
      const response = await this.request<{
        ...
        `/books/${encodeURIComponent(bookId)}/chapters/${encodeURIComponent(
          chapterId,
        )}`,
      );
      ...
      this.error =
        error instanceof Error ? error : new Error("Failed to load chapter.");
    ...
    try {
      ...
      this.error =
        error instanceof Error
          ? error
          : new Error("Failed to load book for chapter.");
