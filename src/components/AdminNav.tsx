  ...
  return (
    <nav className="relative border-t border-[var(--color-border-subtle)] bg-[var(--color-background)]">
      <div className="flex items-center justify-around px-1 pb-5 pt-2.5">
        {[...primaryItems, { page: "admin-more" as Page, label: "More" }].map((item) => {
          ...
          return (
            <button
              key={item.page}
              type="button"
              onClick={() => (isMore ? setMoreOpen((value) => !value) : navigate(item.page))}
            ...
            >
              {isMore ? (
                <MoreHorizontal
                  ...
                  color={moreOpen ? "var(--color-accent-primary)" : "var(--color-text-muted)"}
                />
              ) : (
                <Icon
                  size={20}
              ...
              <span
                ...
                style={{
                  color: active || (isMore && moreOpen) ? "var(--color-accent-primary)" : "var(--color-text-muted)",
                }}
              >
                {item.label}
              </span>
          ...
          {secondaryItems.map((item) => {
            ...
            return (
              <button
                ...
                style={{
                  color: page === item.page ? "var(--color-accent-primary)" : "var(--color-text-secondary)",
                }}
              >
                <Icon size={14} />
                {item.label}
